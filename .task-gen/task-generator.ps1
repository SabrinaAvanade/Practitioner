    ...
    foreach($pbi in $feature.pbis){
        ...
        foreach($attendee in $attendees.attendees){
            ...
            if($attendeeExerciseWorkItem.Count -gt 0){
                ...
                $acceptanceCriteria = $($pbi.'acceptance criteria') -replace '"', '\"'
                ...
                $body = '[{0},{1},{2},{3}]' -f $title, $description, $assignedTo, $acceptanceCriteria
                ...
                foreach($task in $pbi.tasks){
                    ...
                    $attendeeTaskWorkItem = Invoke-RestMethod -Uri $url -headers $authHeader `
                        -Method PATCH `
                        -Body $body `
                        -ContentType 'application/json-patch+json'
